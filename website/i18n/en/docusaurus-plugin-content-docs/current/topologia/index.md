...
This laboratory implements a **Small ISP** architecture with active/backup redundancy between two Nokia 7750 SR-7 BNGs, dual-carrier upstream connectivity via BGP, and access to subscribers through an SR Linux OLT with three different types of service.

## ISP architecture

The topology is made up of three main layers:

:::tip[Layers of Architecture]

- **Carriers**: Two SR Linux routers (Carrier 1 AS 65501, Carrier 2 AS 65502) that provide Internet connectivity with differentiated local-preference (C1=300, C2=150)
- **All in One Solution**: Two BNG Nokia 7750 SR-7 (MASTER/SLAVE) with SRRP, EHS, NAT, ESM and Lawful Interception, plus the services stack (Grafana, Prometheus, Loki, Alloy, gNMIC, RADIUS, LIG, DNS64, Containerbot)
- **Access**: OLT SR Linux connected to ONTs with three service profiles via MAC-VRF

:::
## Containerlab Topology

The lab is defined in the `lab.yml` file:
...
topology:
  nodes:
    ...
    bngmaster:
      kind: nokia_srsim
      image: ghcr.io/abelperezr/srsim:25.10.r2
      mgmt-ipv4: 10.99.1.2
...
| Origin | Port | Destination | Port | Function |
|--------|--------|---------|--------|---------|
...
| BNG MASTER | 1/1/c1/1 | BNG SLAVE | 1/1/c1/1 | IS-IS/LDP/iBGP + SRRP trigger by policy |
| BNG MASTER | 1/1/c2/1 | OLT | ethernet-1/1 | Access (QinQ) + SRRP message-path |
| BNG SLAVE | 1/1/c2/1 | OLT | ethernet-1/2 | Access (QinQ) + SRRP message-path |
