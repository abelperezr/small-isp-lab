---
sidebar_position: 2
sidebar_label: 1. BNG baseline validation
---

# 1. BNG Baseline Validation

:::note[Note]
Apply these checks on the BNGs running SROS.
:::

## 1.1 General Tests

### 1.1.1 Check software version and BOF

Use the following commands to verify the running software version and the BOF parameters:
...
### 1.1.2 Check node inventory

Display the type, model, and serial number of the chassis and installed cards:
...
### 1.1.3 Show system information

Use these commands to review:

- router uptime and last reboot context
- power module status
- active alarms
...
### 1.1.4 Show interface status

Use the following commands to verify the operational status of ports and router interfaces:
...
On the BNG SLAVE, the equivalent routed interface is `to_MASTER`:
...
### 1.1.5 Check system time

Use `show time` to verify the current date and time on the BNGs.

By default, the timezone is set to `est`. You can change it with:
...
Check the active NTP server.

If needed, select the best Google NTP IP according to the region latency. To identify the closest reachable IP, run:
...
That returns the IP that best matches the current region.
...
### 1.1.7 Show router status

Use `show router status` to verify the status of the protocols enabled on the router:
...
Verify that ISIS adjacencies are established between the nodes and that ISIS routes are learned.

### 1.2.1 Verify ISIS interfaces
...
### 1.2.4 Verify the ISIS database
...
Verify that dynamic LDP sessions are created correctly.

### 1.3.1 Verify operational LDP interfaces
...
### 1.3.2 Verify IPv4 LDP sessions
