...
|-----------|-------|
| **Hostname** | carrier1 |
| **Modelo** | Nokia SR Linux 25.10 |
| **IP de Gestión** | 10.99.1.252 |
| **Puerto SSH** | 56610 |
| **ASN** | 65501 |
| **Loopback** | 10.255.1.1/32 |
| **Rol** | Carrier Primario (LP 300 en BNGs) |

## Función en la Topología

Carrier 1 es el proveedor de tránsito **primario**. Los BNGs importan sus rutas con Local-Preference 300 (mayor prioridad). Mantiene sesiones eBGP con ambos BNGs (AS 65520) y anuncia half-default routes (0.0.0.0/1 + 128.0.0.0/1 + ::/1 + 8000::/1) hacia ellos.

La sesión eBGP hacia ambos BNG usa **BFD** para acelerar la detección de fallas.

| Neighbor | IP | Peer AS | Descripción |
|----------|----|---------|-------------|
| BNG MASTER | 172.16.1.0 | 65520 | eBGP IPv4+IPv6 |
| BNG SLAVE | 172.16.1.2 | 65520 | eBGP IPv4+IPv6 |
...
set /interface ethernet-1/1 subinterface 0 ipv4 address 172.16.1.1/31
set /interface ethernet-1/1 subinterface 0 ipv6 admin-state enable
set /interface ethernet-1/1 subinterface 0 ipv6 address 2001:db8:f1::1/126
```
...
set /interface ethernet-1/2 subinterface 0 ipv4 address 172.16.1.3/31
set /interface ethernet-1/2 subinterface 0 ipv6 admin-state enable
set /interface ethernet-1/2 subinterface 0 ipv6 address 2001:db8:f1::5/126
```
...
set /interface ethernet-1/3 subinterface 0 ipv4 address 10.99.100.1/30
set /interface ethernet-1/3 subinterface 0 ipv6 admin-state enable
set /interface ethernet-1/3 subinterface 0 ipv6 address fd00:a1::1/126
```
...
```text
set /network-instance default next-hop-groups group to-gateway nexthop 0 ip-address 10.99.100.2
set /network-instance default next-hop-groups group to-gateway nexthop 0 admin-state enable
set /network-instance default next-hop-groups group to-gateway-v6 nexthop 0 ip-address fd00:a1::2
...
set /network-instance default protocols bgp router-id 10.255.1.1
...
set /network-instance default protocols bgp group to-bngs failure-detection enable-bfd true
...
```text
set /network-instance default protocols bgp neighbor 172.16.1.0 admin-state enable
set /network-instance default protocols bgp neighbor 172.16.1.0 description "to-BNG-MASTER"
set /network-instance default protocols bgp neighbor 172.16.1.0 peer-group to-bngs
set /network-instance default protocols bgp neighbor 172.16.1.2 admin-state enable
set /network-instance default protocols bgp neighbor 172.16.1.2 description "to-BNG-SLAVE"
set /network-instance default protocols bgp neighbor 172.16.1.2 peer-group to-bngs
```
